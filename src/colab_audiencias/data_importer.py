...
class ColabAudienciasPluginDataImporter(PluginDataImporter):
    ...
    def fetch_segment_types(self):
        ...
        for data in json_data:
            segment_type = self.fill_object_data(models.AudienciasSegmentType,
                                                 data)
            segment_type.save()
            all_segment_types.append(segment_type.id)

        models.AudienciasSegmentType.objects.all().exclude(
    ...
    def fetch_bills(self):
        ...
        all_bills = []
        for data in json_data:
            bill = self.fill_object_data(models.AudienciasBill, data)
            bill.save()
            all_bills.append(bill.id)

        models.AudienciasBill.objects.all().exclude(id__in=all_bills).delete()
    ...
    def fetch_segments(self):
        ...
        all_segments = []
        for data in json_data:
            segment = self.fill_object_data(models.AudienciasSegment, data)
            try:
                segment.save()
        ...
        models.AudienciasSegment.objects.all().exclude(
    ...
    def fetch_comments(self):
        ...
        all_comments = []
        for data in json_data:
            comment = self.fill_object_data(models.AudienciasComment, data)
            comment.save()
            all_comments.append(comment.id)

        models.AudienciasComment.objects.all().exclude(
